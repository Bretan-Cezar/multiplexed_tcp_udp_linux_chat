...
int main(int argc, char** argv) 
{
    ...
    // The client receives the number of clients currently connected to the server.
    recv(srv_clnt_tcp, &target_count, sizeof(uint16_t), 0);

    target_count = ntohs(target_count)-1;

    // The client receives the arrival confirmation message.
    memset(buf, 0, 256);
    ...
    while (true) 
    {
        ...
        if (FD_ISSET(0, &curr_rfds_set)) {
            ...
            if (strcmp(msg, "QUIT") == 0) {
                ...
                // Receiving departure confirmation.
                br = recv(srv_clnt_tcp, buf, 256, 0);
